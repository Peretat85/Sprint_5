...
class TestLK:
    def test_click_constructor(self,driver):
        ...
        # переход к Конструктору по клику на кнопку Конструктор
        enter_button = driver.find_element(*Locators.STELLAR_CONSTRUCTOR)
        enter_button.click()

        WebDriverWait(driver, 10).until(EC.url_contains("stellarburgers.nomoreparties.site"))

        # проверка перехода на страницу
        element_profile = driver.find_element(*Locators.STELLAR_CONSTRUCTOR_TEXT)
    ...
    def test_click_logo(self, driver):
        ...
        # переход к Конструктору по клику на кнопку Конструктор
        enter_button = driver.find_element(*Locators.STELLAR_LOGO)
        enter_button.click()
